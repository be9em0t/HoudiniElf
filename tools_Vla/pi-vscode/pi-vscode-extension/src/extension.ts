...
class PiSidebarProvider implements vscode.WebviewViewProvider {
  ...
  private getHtml(webview: vscode.Webview): string {
    ...
    const addIconUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, "media", "BTN_Add.svg")
    ) + `?v=${version}`;
    const sendIconUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, "media", "BTN_Send.svg")
    ) + `?v=${version}`;
    const chevronIconUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, "media", "BTN_chevron.svg")
    ) + `?v=${version}`;
...
function createTools(
  ...
  async function runPlaywrightMcpCommand(args: string[]) {
    try {
