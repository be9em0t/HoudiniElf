# Cluster polygons by overlap and
# order them by area (smaller on top)
...
def fOverlapClustersZorder(layer=None):
    """Cluster polygon features by overlap and assign per-cluster z_order values."""
    if layer is None:
        layer = iface.activeLayer()
...
def fOverlapClustersZorderByZoomlevel(layer=None):
    """Cluster polygon features by overlap and assign z_order using zoomlevel_min."""
    if layer is None:
        layer = iface.activeLayer()
    ...
    order_field = "zoomlevel_min"
    order_idx = layer.fields().indexOf(order_field)
    if order_idx == -1:
        raise ValueError(f"Field '{order_field}' not found on the active layer.")
    ...
    order_values = {f.id(): _as_float(f[order_field]) for f in features}
    ...
    for cid, members in clusters.items():
        members_set = set(members)
        ordered = sorted(members, key=lambda f: (order_values.get(f, 999999.0), f))
        assigned = {}
    ...
    print(f"Done. Found {cluster_counter} overlap clusters; stored IDs in '{cluster_field}' and z-orders in '{z_field}' using '{order_field}'.")
