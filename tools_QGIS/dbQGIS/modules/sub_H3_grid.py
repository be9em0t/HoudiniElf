...
out_name_prefix = "Hex" 
# this shit defines how dence are the tiles samples
# going north requires larger divider (tiles become smaller)
step_divider = 10 
...
def main(extent_layer):
	mylayer, min_max_resolution, geo_csrs, out_csrs = dataPrep(extent_layer)
	geo_layer = proj_to_geo(mylayer)
	buildHexShapes(min_max_resolution, geo_csrs, out_csrs, geo_layer)
	print("Done")
...
def dataPrep(mylayer):
	...
	if ok:
		...
		print(h3level)
		level = int(h3level)
		min_max_resolution = int(h3level)

		# min_resolution = min_max_resolution
		# max_resolution = min_max_resolution
		...
		return mylayer, min_max_resolution, geo_csrs, out_csrs
...
def hexes_within_layer_extent(layer, level):
		...
		# step = max(0.001, (max_lat - min_lat) / (level*level*2))  # adjust step size based on extent
		step = max(0.001, (max_lat - min_lat) / (level*step_divider))  # adjust step size based on extent
...
def buildHexShapes(min_max_resolution, geo_csrs, out_csrs, geo_layer):
	min_resolution = min_max_resolution
	max_resolution = min_max_resolution
