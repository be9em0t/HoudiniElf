...
def main(svrURL, strStateSchema, clipLayer, extentCoords):
	...
	print("Traffic Lights Sub")

	# # fLoadManeuverPts, fLoadManeuverArrow
	newLayer = fLoadTrafLightPoints(mnrServer,mnrSchema,extentCoords)
	# newLayer = fLoadManeuverPts(mnrServer,mnrSchema,extentCoords)
	# newLayer = fLoadManeuverArrow(mnrServer,mnrSchema,extentCoords)
	# # fSymbol(newLayer)


def fLoadTrafLightPoints(mnrServer,mnrSchema,extentCoords):

	# # === db MNR matview ===
	matViewPrefix = "b9" + mnrSchema[0:8] + "_"
	matViewResultTable = matViewPrefix + "traffic_lights"
	extentStr = "'" + extentCoords + "'"

	queryDropLine = "DROP TABLE IF EXISTS " + matViewResultTable + " CASCADE;"

	# juncPtFields = b9PyQGIS.fFieldsFromString("junc.", "feat_id,feat_type,z_level,geom") 
	juncPtFields = b9PyQGIS.fFieldsFromString("junc.", "z_level,geom") 

	lightPtFields = b9PyQGIS.fFieldsFromString("trlightpt.", "feat_type as light_type") 

	metaFieldsType = b9PyQGIS.fFieldsFromString("metatype.", "code_description as junc_desc") # enum_value_id,enum_id,code,code_description
	metaDescriptType = 'Feature Type Junction'


	queryMNRLine =  "CREATE TABLE " + matViewResultTable + " as \
		SELECT " + juncPtFields + "," + lightPtFields + "," + metaFieldsType  +\
		" FROM " + mnrSchema + ".mnr_junction junc " +\
		" LEFT JOIN (" + \
		...
		"  ) as metatype" +\
		" ON junc.feat_type::varchar = metatype.code" +\
		" RIGHT JOIN " + mnrSchema + ".mnr_traffic_light trlightpt" + \
		" ON junc.feat_id = trlightpt.junction_id" +\
		" WHERE " +\
		" ST_Intersects(junc.geom,ST_GeomFromText(" + extentStr + ",4326)) ;" 
		# +\
